...
!*****************************************************************************************
    use type_list
    implicit none
    character*5 :: ID_tmp, ID_u_tmp, ID_s_tmp
    ...
    CHARACTER :: name*10, name1*99, file*30, dir*99
...
!    PRINT*, ASSOCIATED(ID)
    ...
    file = 'ID_list.txt'
    dir = 'H:/work2/SPBGK/SPBGK20U1/ID'
    ...
    DO WHILE (ASSOCIATED(tmpptr))
      ...
      OPEN(UNIT=10, STATUS='OLD', FILE=name1, FORM='UNFORMATTED', CONVERT='LITTLE_ENDIAN')
        ...
        IF (sz <= 113) THEN
          READ(10) kn_tmp, number_unit_tmp, L_r_tmp, imax1_tmp, M_ref_tmp, M_z_tmp, M_z_th_tmp, &
                   ...
                   coeff_under_inf_tmp, ID_aitken_tmp, step_aitken_tmp, d1g_tmp, d2g_tmp, d3g_tmp
          ID_es_tmp = 0
          nu_tmp = 0.d0
          ID_dev_noneq_tmp = 'n'
          ID_dsc_analyt_tmp = 'y'
        ...
        ELSE
          READ(10) kn_tmp, number_unit_tmp, L_r_tmp, imax1_tmp, M_ref_tmp, M_z_tmp, M_z_th_tmp, &
                   ...
                   n_crit_conv_tmp, step_add_conv_tmp, ID_dev_tmp, ID_dev_level_tmp, &
                   coeff_under_inf_tmp, ID_aitken_tmp, step_aitken_tmp, d1g_tmp, d2g_tmp, d3g_tmp, &
                   ID_es_tmp, nu_tmp, ID_dev_noneq_tmp
          ID_dsc_analyt_tmp = 'y'
        ENDIF
      CLOSE(10)
      tmpptr%kn = kn_tmp
...
!*****************************************************************************************
    ! no need for sorting
    IF (ASSOCIATED(x) == .FALSE. .OR. ASSOCIATED(x%next) == .FALSE.) THEN
      z => x
      RETURN
    ENDIF
...
!*****************************************************************************************
    ...
    DO WHILE (ASSOCIATED(currptr))
      PRINT*, currptr%ID_name, SNGL(currptr%kn)
...
!    WRITE(*,*) 'Data written to ...', TRIM(name)
    OPEN(UNIT=10, STATUS='unknown', FILE=name)
      WRITE(10,'(X,A2)',advance='no') 'ID'
      write(10,'(X,A6)',advance='no') 'kn'
      ...
      write(10,'(X,A)',advance='no') 'IDfm'
      write(10,'(X,A)',advance='no') 'ID_dsc'
      write(10,'(X,A)',advance='no') 'ID_dsc_analyt'
      write(10,'(X,A)',advance='no') 'ID_inf'
      write(10,'(X,A)',advance='no') 'n_crit_conv'
      write(10,'(X,A)',advance='no') 'step_add_conv'
      write(10,'(X,A)',advance='no') 'ID_dev'
      write(10,'(X,A)',advance='no') 'ID_dev_level'
      write(10,'(X,A)',advance='no') 'ID_dev_noneq'
      write(10,'(X,A)',advance='no') 'coeff_under_inf'
      write(10,'(1X,A)',advance='no') 'ID_es'
      write(10,'(3X,A)',advance='yes') 'nu'

      WRITE(10,'(A)') REPEAT('-',212)
      DO WHILE (ASSOCIATED(currptr))
        write(10,'(X,A4)',advance='no') currptr%ID_name
        ...
        write(10,'(X,I6)',advance='no') currptr%M_z
        ...
        write(10,'(X,A4)',advance='no') currptr%IDfm
        write(10,'(X,A4)',advance='no') currptr%ID_dsc
        ...
        write(10,'(X,I5)',advance='no') currptr%ID_es
...
!    k0 = -1.2540d0
    ...
    open(unit=10, status='unknown', file=name)
      write(10,'(X,A)') 'VARIABLES = "kn", "hl"'

      if (currptr%ID_es == 0) then
        write(10,*) 'ZONE T = "num (BGK)"'
      elseif (currptr%ID_es == 1) then
        write(10,*) 'ZONE T = "num (ES)"'
      endif
